            ...
            <template v-for="(data,i) in tableData">
                <swipeout-item :key="i" @on-close="handleEvents('on-close')" @on-open="handleEvents('on-open')" transition-mode="follow">
                  ...
                  <div slot="content" class="demo-content">
	                  <card :header="{title: data.content}">
                     <div slot="content" class="card-demo-flex card-demo-content01">
                       ...
                       <div class="vux-1px-r">
                         <span> {{isNaN(data.commissionDate)?data.commissionDate.length>10?data.commissionDate:'每'+data.commissionDate:'每月'+data.commissionDate+'日'}}</span>
                         <br/>
                         待办日期
                       </div>
                       <div class="vux-1px-r">
                         <span>{{data.periodicTypeStr}}</span>
                         <br/>
                        事务类型
                       </div>
                       <div>
                         <span>{{data.commissionUser}}</span>
                         <br/>
                         代办人
                        </div>
                      </div>
...
export default {
  ...
  methods: {
    ...
    onButtonClick(data) {
      ...
      this.$router.push(`/affair/${data.id}`);
    },
    onButtonDelete(data) {
      this.dataId = data.id;
      this.show3 = true;
